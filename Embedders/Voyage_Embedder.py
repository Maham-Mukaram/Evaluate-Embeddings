...
class VoyageEmbedder:
    """
    Class for embedding queries and corpus using VoyageAI embedding model.
    """
    def __init__(self, engine, task_name=None, batch_size=32, sep=" ", save_emb=False, API_KEY="V", base_path='', **kwargs):
        """
        Initialize the VoyageEmbedder object.

        Parameters:
        - engine (str): VoyageAI embedding model.
    ...
    def encode_queries(self, queries: List[str], **kwargs):
        """
        Encode a list of queries using the VoyageAI API.
        ...
        if queries and os.path.exists(embedding_path):
            ...
            # If not, retrieve embeddings from VoyageAI API in batches
            for i in range(0, len(queries), self.batch_size):
                batch = queries[i : i + self.batch_size]
                response = voyageai.get_embeddings(batch, model=self.engine, input_type="query")
    ...
    def encode_corpus(self, corpus: List[Dict[str, str]], **kwargs):
        """
        Encode a list of documents in the corpus using the VoyageAI API.
        ...
        if sentences and os.path.exists(embedding_path):
            ...
            for i in range(0, len(sentences), self.batch_size):
                ...
                response = voyageai.get_embeddings(batch, model=self.engine, input_type="document")
                fin_embeddings.extend(response)
