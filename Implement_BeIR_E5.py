...
class E5Embedder:
    ...
    def encode_queries(self, queries: List[str], batch_size: int, **kwargs):
        ...
        filename = f"2_{self.task_name}_{queries[0][:10]}_{queries[-1][-10:]}"
...
def parse_args():
    ...
    parser.add_argument("--engine", type=str, default='intfloat/e5-base-v2')
...
def main(args):
    ...
    for task in TASK_LIST[args.startid:args.endid]:
        ...
        if os.path.exists(save_path):
            ...
            corpus, queries, qrels = GenericDataLoader(data_folder=data_path, query_file='queries.jsonl').load(split=eval_splits)
            model = DRES(E5Embedder
