...
class GTEEmbedder:
    ...
    def encode_queries(self, queries: List[str], batch_size: int, **kwargs):
        ...
        filename = f"Q2E_2_{self.task_name}_{queries[0][:10]}_{queries[-1][-10:]}"
...
def parse_args():
    ...
    parser.add_argument("--engine", type=str, default='thenlper/gte-base')
...
def main(args):
    ...
    result_path = os.path.join(f"BeIR_Results/{model_name}/Q2E_2")
    ...
    for task in TASK_LIST[args.startid:args.endid]:
        ...
        if os.path.exists(save_path):
            ...
            corpus, queries, qrels = GenericDataLoader(data_folder=data_path, query_file='expanded_queries/queries_Q2E.jsonl').load(split=eval_splits)
            model = DRES(GTEEmbedder
